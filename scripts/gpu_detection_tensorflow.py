...
class ObjectDetectionTF():
    ...
    def __init__(self):
		...
		with self.detection_graph.as_default():
		    ...
		    with tf.gfile.GFile(PATH_TO_CKPT, 'rb') as fid:
		        ...
		        #self.sess = tf.Session(config=tf.ConfigProto(log_device_placement=True)) # to see which device has the variabled and some operations 
    ...
    # Actual detection.
		t1= time.time()
        	(boxes, scores, classes, num, masks) = self.sess.run([detection_boxes, detection_scores, detection_classes, num_detections,detection_masks], feed_dict={image_tensor: image_np_expanded})
		t2= time.time()
		print('>>>>>> time for GPU model to execute >>>>>',t2-t1)
